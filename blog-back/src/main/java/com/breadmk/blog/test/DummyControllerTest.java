...
//	{id} 주소로 파라미터를 전달 받을 수 있음.
	...
	public User detail(@PathVariable int id) {
		// user/4 을 찾으면 내가 데이터베이스에서 못 찾아오면 user가 null이 될거 아니야? 그럼 return 할때 null 이 return 되니 문제가 발생한다.
		...
		// 만약에 자바 오브젝트를 return 하게 되면 MessageConverter가 Jackson 이라는 라이브러리를 호출해서
		// user 오브젝트를 json으로 변환해서 브라우저에게 던져준다.
		return user;
	}

	//http의 body에 usernam,password,email 데이터를 가지고 (요청)
